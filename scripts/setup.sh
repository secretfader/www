...
LATEST_VERSION="select(.draft == false) | select(.prerelease == false)
    .assets[] |
        select(.name | contains(\"extended\")) |
        select(.name | contains(\"Linux-64bit.tar.gz\"))"

DOWNLOAD_URL="(.[0] | $LATEST_VERSION).browser_download_url"

# Retrieve the latest stable, extended release version of from GitHub
HUGO_DOWNLOAD_URL=$(printf "%s" "$HUGO_RELEASES" | jq -r "$DOWNLOAD_URL")
